...
| 真实网页登录普通聊天 | 无敏感内容的小请求返回文本 | 环境阻塞：Qwen 模型目录通过但会话创建返回空的非 JSON 响应；DeepSeek/ds2api 返回 503 `upstream_unavailable`。现有登录态需重新授权或等待网页账号恢复，不能记为通过 |
| 真实工具闭环 | 标准 tool call，回传工具结果后得到最终回答 | 环境阻塞：两条 provider 的文本调用均未建立，故 OpenAI/Anthropic tool-use 与 tool-result smoke 不能记为通过 |
| 最新代码启动 | 重启本机 Gateway | 已通过：代码提交 `a362783` 已推送 `origin/main`，Gateway 启动并监听 PID `71676` |
| 健康检查 | `/health`: `ok=true`、`runtime.sourceFresh=true`、`runtime.sourceStale=false` | 已通过：三项值分别为 `true / true / false`；supervisor `failedServices=[]`，WebAI2API 与 ds2api 状态均为 `running` |
